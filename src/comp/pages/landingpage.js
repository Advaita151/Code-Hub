...
function Landingpage() {
  ...
  function getNextLeetCodeContest() {
    ...
    if (nextSunday > today) {
        return nextSunday.toLocaleDateString('en-US', { month: 'numeric', day: 'numeric', year: 'numeric' });
    }
  ...
  function calculateNextWednesday() {
    const today = new Date();
    const daysUntilWednesday = 3 - today.getDay();
    const nextWednesdayDate = new Date(today.getTime() + daysUntilWednesday * 24 * 60 * 60 * 1000);
    setNextWednesday(nextWednesdayDate.toLocaleDateString());
  }
        ...
        <div className="bg-white p-4 rounded-md shadow-md mb-4 flex flex-col gap-1 h-50 w-48">
          ...
          <h3 className="text-xl font-bold mb-2">Codeforces</h3>
          {
            nextContest ? (
              ...
              <p className="text-gray-600">Next is on: <br /> {new Date(nextContest.startTimeSeconds * 1000).toLocaleString()}</p>
              ) :(
